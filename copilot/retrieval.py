...
def load_markdown_documents(path: str | Path) -> list[Document]:
    ...
    for file_path in sorted(Path(path).glob("*.md")):
        ...
        first_line = next((line.strip("# ").strip() for line in text.splitlines() if line.startswith("#")), file_path.stem)
...
class TfIdfRetriever:
    ...
    def _build_idf(self) -> dict[str, float]:
        ...
        return {
            term: math.log((1 + total) / (1 + sum(term in counts for counts in self._doc_tokens))) + 1
